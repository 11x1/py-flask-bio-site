...
def register_page():
    ...
    if request.method == 'POST':
        ...
        if not password_repeat == password: error = 'passwods do not match'
            ...
            db = login_and_return_db()
            db_cursor = db.cursor()
            rows = db_cursor.execute('use webdev')
            rows = db_cursor.execute('select count(*) from users')
            db_cursor.execute(f'INSERT INTO users (userid, username, password, profile_picture, description) VALUES ({rows + 1}, {username}, {password}, {0}, {0})')
            db.commit()
            return redirect('login')
