...
@app.route('/users/<username>.<userid>')
def userlist_page(username, userid):
    if not 'username' in session: return redirect('login')

    return render_template('profile.html', username=session['username'], uid=session['uid'], profile_user=username, profile_userid=userid)
